...
for i=1:length(S.Nodes)
    ...
    tmpNode=textscan(S.Nodes{i}.name,'%s %d,');
    maxNodeNum=max(maxNodeNum,tmpNode{2});
    % reindex node number
    if tmpNode{2}~=i
...
diffNodeNum2=zeros(1,maxNodeNum);
for i=1:length(diffNodeNum)
    if (diffNodeNum(i)~=0)
        diffNodeNum2(i-diffNodeNum(i))=diffNodeNum(i);
    end
end
S=tmpS;
...
for i=1:length(S.Vessels)
    ...
    elseif length(S.Vessels{i}.connections)==2
        for j=1:2
            ...
            if diffNodeNum2(tmpNodeIdx)~=0
                S.Vessels{i}.connections{j}=tmpNodeIdx+diffNodeNum2(tmpNodeIdx);
            end
        end
