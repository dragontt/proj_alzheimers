function [SS,maskDel]=BuildTmpSkel2(S,flagDelShortSkel,threshold)
...
for i=1:length(SS)
    for j=1:length(SS)
        if i~=j
            if (pdist2(SS(i).StartPt,SS(j).StartPt)<10 || pdist2(SS(i).StartPt,SS(j).EndPt)<10)
                SS(i).SkelConn2StartPt=[SS(i).SkelConn2StartPt,j];
            elseif (pdist2(SS(i).EndPt,SS(j).StartPt)<10 || pdist2(SS(i).EndPt,SS(j).EndPt)<10)
