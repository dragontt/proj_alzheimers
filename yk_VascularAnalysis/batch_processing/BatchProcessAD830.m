% batch process AD 830 Combined AB stacks

close all; clear; clc;

cd ..;
addpath(genpath('./functions/'));
batchMorph=cell(1,7); % last column is for the unstalled vessel
batchSpath=cell(2,7); % first row shortest path of branching; second row shortest path of length
numStalled=zeros(1,6);
...
%% day 0 pre
load('/Users/schafferlab/Documents/MATLAB/yk_Data/AD Analysis/AD 830/Combined AB/day 0 pre/Analysis.mat');
...
%% day 0 post
load('/Users/schafferlab/Documents/MATLAB/yk_Data/AD Analysis/AD 830/Combined AB/day 0 post/Analysis.mat');
...
%% day 1
load('/Users/schafferlab/Documents/MATLAB/yk_Data/AD Analysis/AD 830/Combined AB/day 1/Analysis.mat');
...
for i=1:2
    ...
    tmp(find(tmp==0))=[];
    batchSpath{1,7}{i}=tmp;
    tmp=spathListLength{1}(i,:);
    tmp(find(tmp==0))=[];
    batchSpath{2,3}{i}=tmp;
    tmp=spathListLength{2}(i,:);
    tmp(find(tmp==0))=[];
    batchSpath{2,7}{i}=tmp;
end
numStalled(3)=size(spathStalled,2);

%% day 3
load('/Users/schafferlab/Documents/MATLAB/yk_Data/AD Analysis/AD 830/Combined AB/day 3/Analysis.mat');
...
%% day 7
load('/Users/schafferlab/Documents/MATLAB/yk_Data/AD Analysis/AD 830/Combined AB/day 7/Analysis.mat');

[spathStatsBranch{1},spathListBranch{1}]=ComptSpathStatsBranchNum(spathStalled(1,:),paramTop_numBins,paramTop_maxBranchNum);
[spathStatsLength{1},spathListLength{1}]=ComptSpathStatsVessLength2(spathStalled(2,:),lengthListStalled,paramTop_numBins,paramTop_maxVessLength);

batchMorph{5}=morphStalled;
for i=1:2
    tmp=spathListBranch{1}(i,:);
    tmp(find(tmp==0))=[];
...
numStalled(5)=size(spathStalled,2);

%% day 14
load('/Users/schafferlab/Documents/MATLAB/yk_Data/AD Analysis/AD 830/Combined AB/day 14/Analysis.mat');

[spathStatsBranch{1},spathListBranch{1}]=ComptSpathStatsBranchNum(spathStalled(1,:),paramTop_numBins,paramTop_maxBranchNum);
[spathStatsLength{1},spathListLength{1}]=ComptSpathStatsVessLength2(spathStalled(2,:),lengthListStalled,paramTop_numBins,paramTop_maxVessLength);

batchMorph{6}=morphStalled;
for i=1:2
    tmp=spathListBranch{1}(i,:);
    tmp(find(tmp==0))=[];
    batchSpath{1,6}{i}=tmp;
    tmp=spathListLength{1}(i,:);
    tmp(find(tmp==0))=[];
    batchSpath{2,6}{i}=tmp;
end
numStalled(6)=size(spathStalled,2);

%% view plots
figure(1);
plot(numStalled); hold on; scatter(1:6,numStalled); hold off;
axis([1,6,0,max(numStalled)]);
set(gca,'XTick',1:6);
set(gca,'XTickLabel',{'day0 pre','day0 post','day1','day3','day7','day14'});
...
for i=1:6
    ...
    boxplot([batchMorph{1}(:,i+1);batchMorph{2}(:,i+1);batchMorph{3}(:,i+1);...
        batchMorph{4}(:,i+1);batchMorph{5}(:,i+1);batchMorph{6}(:,i+1);...
        batchMorph{7}(:,i+1);],...
        [zeros(size(batchMorph{1},1),1);ones(size(batchMorph{2},1),1);...
        2*ones(size(batchMorph{3},1),1);3*ones(size(batchMorph{4},1),1);...
        4*ones(size(batchMorph{5},1),1);5*ones(size(batchMorph{6},1),1);...
        6*ones(size(batchMorph{7},1),1)],'labels',{'day0 pre','day0 post',...
        'day1','day3','day7','day14','unstalled'},'jitter',0,'whisker',5);
    title(titleArray{i});
end
set(gcf, 'Color', 'White');
...
boxplot([batchSpath{1,1}{1},batchSpath{1,2}{1},batchSpath{1,3}{1},...
    ...
    4*ones(1,length(batchSpath{1,5}{1})),5*ones(1,length(batchSpath{1,6}{1})),...
    6*ones(1,length(batchSpath{1,7}{1}))],'labels',{'day0 pre','day0 post',...
        'day1','day3','day7','day14','unstalled'},'jitter',0,'whisker',1.5);
...
boxplot([batchSpath{1,1}{2},batchSpath{1,2}{2},batchSpath{1,3}{2},...
    batchSpath{1,4}{2},batchSpath{1,5}{2},batchSpath{1,6}{2},batchSpath{1,7}{2}],...
    ...
    4*ones(1,length(batchSpath{1,5}{2})),5*ones(1,length(batchSpath{1,6}{2})),...
    6*ones(1,length(batchSpath{1,7}{2}))],'labels',{'day0 pre','day0 post',...
        'day1','day3','day7','day14','unstalled'},'jitter',0,'whisker',1.5);
...
boxplot([batchSpath{2,1}{1},batchSpath{2,2}{1},batchSpath{2,3}{1},...
    batchSpath{2,4}{1},batchSpath{2,5}{1},batchSpath{2,6}{1},batchSpath{2,7}{1}],...
    ...
    4*ones(1,length(batchSpath{2,5}{1})),5*ones(1,length(batchSpath{2,6}{1})),...
    6*ones(1,length(batchSpath{2,7}{1}))],'labels',{'day0 pre','day0 post',...
        'day1','day3','day7','day14','unstalled'},'jitter',0,'whisker',1.5);
...
boxplot([batchSpath{2,1}{2},batchSpath{2,2}{2},batchSpath{2,3}{2},...
    batchSpath{2,4}{2},batchSpath{2,5}{2},batchSpath{2,6}{2},batchSpath{2,7}{2}],...
    ...
    4*ones(1,length(batchSpath{2,5}{2})),5*ones(1,length(batchSpath{2,6}{2})),...
    6*ones(1,length(batchSpath{2,7}{2}))],'labels',{'day0 pre','day0 post',...
        'day1','day3','day7','day14','unstalled'},'jitter',0,'whisker',1.5);
